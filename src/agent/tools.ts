...
export async function executeLocalTool(tool: ToolCall): Promise<string> {
  ...
  switch (name) {
    ...
    case "str_replace_file": {
      ...
      if (!content.includes(input.oldStr)) return `Error: oldStr not found in ${input.path}`;
      const count = content.split(input.oldStr).length - 1;
      if (count > 1) return `Error: oldStr found ${count} times in ${input.path}. Make it more specific.`;
      fs.writeFileSync(resolve(input.path), content.replace(input.oldStr, input.newStr));
      return `Replaced in ${input.path}`;
    }
    case "append_file": fs.appendFileSync(resolve(input.path), input.content); return `Appended to ${input.path}`;
    ...
    case "curl_request": {
      ...
      if (input.headers) cmd += ` -H '${input.headers}'`;
      if (input.body) cmd += ` -d '${input.body}'`;
